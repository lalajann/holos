...
function setup() {
    ...
    skiltis = createElement('h2', 'STOCK MARKET')
...
function showStatistika() {
    parent = createDiv('company' + ' / ' + 'traded volume')
...
function draw() {
    ...
    for (var j = 0; j < pasiskirstymas[0]; j++) {
        let a = angle + offset;
        let h = floor(map(sin(a), -1, 1, 200, 600))
        translate(j * 9, 0, 0)
        fill(100, 30, 255);
        noStroke();
        rotateY(120);
    ...
    for (var j = 0; j < pasiskirstymas[2]; j++) {
        ...
        strokeWeight(2);
        rotateZ(mouseX / 10000);

        rotate(mapTreble / 2);
        circle(j * 5, radius * 4, 12);
    }
    for (var j = 0; j < pasiskirstymas[3]; j++) {
        ...
        rotateZ(mouseX / 10000);
        rotate(mapTreble / 2);
        line(j * offset, radius * 4 + 50, j * offset, radius * 4 + 100);
    }
    // PENKI
    ...
    for (var j = 0; j < pasiskirstymas[8]; j++) {
        ...
        rotate(mapTreble / 2);
        rotateZ(mouseX / 10000);
        line(j * offset, radius * 4 + 650, j * offset, radius * 4 + 700);
    }
}
